...
schedule(void)
{
	...
	if (scheduling_algorithm == 0) {
        ...
        // They're ordered in the processor array
        while (1) {
            for (pid=0;pid<NPROCS;pid++) {
                if (proc_array[pid].p_state == P_RUNNABLE) run(&proc_array[pid]);
            }
        }
    } else if (scheduling_algorithm == 2) {
        while (1) {
            // find value of highest priority
            for (pid_t i=0;i<NPROCS;i++) {
                if (proc_array[i].p_state == P_RUNNABLE && highest_priority > proc_array[i].p_priority) highest_priority = proc_array[i].p_priority;
            }
            // find process with highest priority and run
            pid = (pid + 1) % NPROCS;
            if (proc_array[pid].p_state == P_RUNNABLE &&
                (proc_array[pid].p_priority < highest_priority || proc_array[pid].p_priority == highest_priority)) run(&proc_array[pid]);
        }
    }
